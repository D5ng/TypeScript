...
function addNumber2(n1: number, n2: number): number {
  return n1 + n2;
}

// ! 함수는 반환할 값이 없을 수 있다.
function voidInterface(n1: number, n2: number) {
  console.log(n1 + n2);
}

// ! 함수는 값을 반환할 수 없다는것을 타입 표기로 지정할 수 있다.
function voidAnnotation(n1: number, n2: number): void {
  console.log(n1 + n2);
}
...
function printNumber(callback: (index: number) => void) {
  for (let i = 1; i <= 10; i++) {
    callback(i);
  }
}
